...
(async () => {
  ...
  // Prefer IPv4 loopback to avoid IPv6/localhost resolution mismatches on Windows
  const url = process.env.E2E_BASE || 'http://127.0.0.1:5173/admin-dashboard?dev_admin=1';
  console.log('Navigating to', url);
  try {
    await page.goto(url, { waitUntil: 'networkidle' });
  } catch (err) {
    console.warn('Initial navigation failed, retrying in 1s...', err.message || err);
